...
public class FFmpegKitConfig {
    ...
    private static com.antonkarpenko.ffmpegkit.Level activeLogLevel;

    /* Session history variables */
    private static int sessionHistorySize;
    private static final Map<Long, com.antonkarpenko.ffmpegkit.Session> sessionHistoryMap;
    private static final List<com.antonkarpenko.ffmpegkit.Session> sessionHistoryList;
    ...
    /* Global callbacks */
    private static com.antonkarpenko.ffmpegkit.LogCallback globalLogCallback;
    private static com.antonkarpenko.ffmpegkit.StatisticsCallback globalStatisticsCallback;
    private static com.antonkarpenko.ffmpegkit.FFmpegSessionCompleteCallback globalFFmpegSessionCompleteCallback;
    private static com.antonkarpenko.ffmpegkit.FFprobeSessionCompleteCallback globalFFprobeSessionCompleteCallback;
    private static com.antonkarpenko.ffmpegkit.MediaInformationSessionCompleteCallback globalMediaInformationSessionCompleteCallback;
    ...
    private static com.antonkarpenko.ffmpegkit.LogRedirectionStrategy globalLogRedirectionStrategy;

    static {

//        Exceptions.registerRootPackage("com.antonkarpenko");

        android.util.Log.i(FFmpegKitConfig.TAG, "Loading ffmpeg-kit.");

        final boolean nativeFFmpegTriedAndFailed = com.antonkarpenko.ffmpegkit.NativeLoader.loadFFmpeg();

        /* ALL FFMPEG-KIT LIBRARIES LOADED AT STARTUP */
        com.antonkarpenko.ffmpegkit.Abi.class.getName();
        com.antonkarpenko.ffmpegkit.FFmpegKit.class.getName();
        com.antonkarpenko.ffmpegkit.FFprobeKit.class.getName();

        com.antonkarpenko.ffmpegkit.NativeLoader.loadFFmpegKit(nativeFFmpegTriedAndFailed);

        uniqueIdGenerator = new AtomicInteger(1);

        /* NATIVE LOG LEVEL IS RECEIVED ONLY ON STARTUP */
        activeLogLevel = com.antonkarpenko.ffmpegkit.Level.from(com.antonkarpenko.ffmpegkit.NativeLoader.loadLogLevel());

        asyncConcurrencyLimit = 10;
        asyncExecutorService = Executors.newFixedThreadPool(asyncConcurrencyLimit);

        sessionHistorySize = 10;
        sessionHistoryMap = new LinkedHashMap<Long, com.antonkarpenko.ffmpegkit.Session>() {

            @Override
            protected boolean removeEldestEntry(Map.Entry<Long, com.antonkarpenko.ffmpegkit.Session> eldest) {
        ...
        globalLogRedirectionStrategy = com.antonkarpenko.ffmpegkit.LogRedirectionStrategy.PRINT_LOGS_WHEN_NO_CALLBACKS_DEFINED;

        android.util.Log.i(FFmpegKitConfig.TAG, String.format("Loaded ffmpeg-kit-%s-%s-%s-%s.", com.antonkarpenko.ffmpegkit.NativeLoader.loadPackageName(), com.antonkarpenko.ffmpegkit.NativeLoader.loadAbi(), com.antonkarpenko.ffmpegkit.NativeLoader.loadVersion(), com.antonkarpenko.ffmpegkit.NativeLoader.loadBuildDate()));
    }
    ...
    private static void log(final long sessionId, final int levelValue, final byte[] logMessage) {
        final com.antonkarpenko.ffmpegkit.Level level = com.antonkarpenko.ffmpegkit.Level.from(levelValue);
        final String text = new String(logMessage);
        final com.antonkarpenko.ffmpegkit.Log log = new com.antonkarpenko.ffmpegkit.Log(sessionId, level, text);
        boolean globalCallbackDefined = false;
        boolean sessionCallbackDefined = false;
        com.antonkarpenko.ffmpegkit.LogRedirectionStrategy activeLogRedirectionStrategy = globalLogRedirectionStrategy;

        // AV_LOG_STDERR logs are always redirected
        if ((activeLogLevel == com.antonkarpenko.ffmpegkit.Level.AV_LOG_QUIET && levelValue != com.antonkarpenko.ffmpegkit.Level.AV_LOG_STDERR.getValue()) || levelValue > activeLogLevel.getValue()) {
            // LOG NEITHER PRINTED NOR FORWARDED
            return;
        }

        final com.antonkarpenko.ffmpegkit.Session session = getSession(sessionId);
        if (session != null) {
...
//                    android.util.Log.e(FFmpegKitConfig.TAG, String.format("Exception thrown inside session log callback.%s", Exceptions.getStackTraceString(e)));
        ...
        final com.antonkarpenko.ffmpegkit.LogCallback globalLogCallbackFunction = FFmpegKitConfig.globalLogCallback;
...
//                android.util.Log.e(FFmpegKitConfig.TAG, String.format("Exception thrown inside global log callback.%s", Exceptions.getStackTraceString(e)));
    ...
    private static void statistics(final long sessionId, final int videoFrameNumber,
        ...
        final com.antonkarpenko.ffmpegkit.Statistics statistics = new com.antonkarpenko.ffmpegkit.Statistics(sessionId, videoFrameNumber, videoFps, videoQuality, size, time, bitrate, speed);

        final com.antonkarpenko.ffmpegkit.Session session = getSession(sessionId);
        if (session != null && session.isFFmpeg()) {
            com.antonkarpenko.ffmpegkit.FFmpegSession ffmpegSession = (com.antonkarpenko.ffmpegkit.FFmpegSession) session;
...
//                    android.util.Log.e(FFmpegKitConfig.TAG, String.format("Exception thrown inside session statistics callback.%s", Exceptions.getStackTraceString(e)));
        ...
        final com.antonkarpenko.ffmpegkit.StatisticsCallback globalStatisticsCallbackFunction = FFmpegKitConfig.globalStatisticsCallback;
...
//            android.util.Log.e(TAG, String.format("Failed to set font directory: %s.%s", Arrays.toString(fontDirectoryList.toArray()), Exceptions.getStackTraceString(e)));
    ...
    public static List<String> getSupportedCameraIds(final Context context) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            detectedCameraIdList.addAll(com.antonkarpenko.ffmpegkit.CameraSupport.extractSupportedCameraIds(context));
        }
    ...
    public static boolean isLTSBuild() {
        return com.antonkarpenko.ffmpegkit.AbiDetect.isNativeLTSBuild();
    }
     ...
     */
    public static void ignoreSignal(final com.antonkarpenko.ffmpegkit.Signal signal) {
     ...
     */
    public static void ffmpegExecute(final com.antonkarpenko.ffmpegkit.FFmpegSession ffmpegSession) {
        ffmpegSession.startRunning();

        try {
            final int returnCode = nativeFFmpegExecute(ffmpegSession.getSessionId(), ffmpegSession.getArguments());
            ffmpegSession.complete(new com.antonkarpenko.ffmpegkit.ReturnCode(returnCode));
        } catch (final Exception e) {
...
//            android.util.Log.w(FFmpegKitConfig.TAG, String.format("FFmpeg execute failed: %s.%s", FFmpegKitConfig.argumentsToString(ffmpegSession.getArguments()), Exceptions.getStackTraceString(e)));
     ...
     */
    public static void ffprobeExecute(final com.antonkarpenko.ffmpegkit.FFprobeSession ffprobeSession) {
        ...
        try {
            ...
            ffprobeSession.complete(new com.antonkarpenko.ffmpegkit.ReturnCode(returnCode));
        } catch (final Exception e) {
...
//            android.util.Log.w(FFmpegKitConfig.TAG, String.format("FFprobe execute failed: %s.%s", FFmpegKitConfig.argumentsToString(ffprobeSession.getArguments()), Exceptions.getStackTraceString(e)));
     ...
     */
    public static void getMediaInformationExecute(final com.antonkarpenko.ffmpegkit.MediaInformationSession mediaInformationSession, final int waitTimeout) {
        ...
        try {
            ...
            final com.antonkarpenko.ffmpegkit.ReturnCode returnCode = new com.antonkarpenko.ffmpegkit.ReturnCode(returnCodeValue);
            mediaInformationSession.complete(returnCode);
            if (((com.antonkarpenko.ffmpegkit.ReturnCode) returnCode).isValueSuccess()) {
                List<com.antonkarpenko.ffmpegkit.Log> allLogs = mediaInformationSession.getAllLogs(waitTimeout);
                final StringBuilder ffprobeJsonOutput = new StringBuilder();
                for (int i = 0, allLogsSize = allLogs.size(); i < allLogsSize; i++) {
                    com.antonkarpenko.ffmpegkit.Log log = allLogs.get(i);
                    if (log.getLevel() == com.antonkarpenko.ffmpegkit.Level.AV_LOG_STDERR) {
                        ffprobeJsonOutput.append(log.getMessage());
                    }
                }
                com.antonkarpenko.ffmpegkit.MediaInformation mediaInformation = com.antonkarpenko.ffmpegkit.MediaInformationJsonParser.fromWithError(ffprobeJsonOutput.toString());
...
//            android.util.Log.w(FFmpegKitConfig.TAG, String.format("Get media information execute failed: %s.%s", FFmpegKitConfig.argumentsToString(mediaInformationSession.getArguments()), Exceptions.getStackTraceString(e)));
     ...
     * @param ffmpegSession FFmpeg session which includes command options/arguments
     */
    public static void asyncFFmpegExecute(final com.antonkarpenko.ffmpegkit.FFmpegSession ffmpegSession) {
        com.antonkarpenko.ffmpegkit.AsyncFFmpegExecuteTask asyncFFmpegExecuteTask = new com.antonkarpenko.ffmpegkit.AsyncFFmpegExecuteTask(ffmpegSession);
     ...
     */
    public static void asyncFFmpegExecute(final com.antonkarpenko.ffmpegkit.FFmpegSession ffmpegSession, final ExecutorService executorService) {
        com.antonkarpenko.ffmpegkit.AsyncFFmpegExecuteTask asyncFFmpegExecuteTask = new com.antonkarpenko.ffmpegkit.AsyncFFmpegExecuteTask(ffmpegSession);
     ...
     * @param ffprobeSession FFprobe session which includes command options/arguments
     */
    public static void asyncFFprobeExecute(final com.antonkarpenko.ffmpegkit.FFprobeSession ffprobeSession) {
        com.antonkarpenko.ffmpegkit.AsyncFFprobeExecuteTask asyncFFmpegExecuteTask = new com.antonkarpenko.ffmpegkit.AsyncFFprobeExecuteTask(ffprobeSession);
        Future<?> future = asyncExecutorService.submit(asyncFFmpegExecuteTask);
     ...
     * @param executorService executor service that will be used to run this asynchronous operation
     */
    public static void asyncFFprobeExecute(final com.antonkarpenko.ffmpegkit.FFprobeSession ffprobeSession, final ExecutorService executorService) {
        com.antonkarpenko.ffmpegkit.AsyncFFprobeExecuteTask asyncFFmpegExecuteTask = new com.antonkarpenko.ffmpegkit.AsyncFFprobeExecuteTask(ffprobeSession);
        Future<?> future = executorService.submit(asyncFFmpegExecuteTask);
        ffprobeSession.setFuture(future);
    }
     ...
     * @param waitTimeout             max time to wait until media information is transmitted
     */
    public static void asyncGetMediaInformationExecute(final com.antonkarpenko.ffmpegkit.MediaInformationSession mediaInformationSession, final int waitTimeout) {
        com.antonkarpenko.ffmpegkit.AsyncGetMediaInformationTask asyncGetMediaInformationTask = new com.antonkarpenko.ffmpegkit.AsyncGetMediaInformationTask(mediaInformationSession, waitTimeout);
     ...
     * @param waitTimeout             max time to wait until media information is transmitted
     */
    public static void asyncGetMediaInformationExecute(final com.antonkarpenko.ffmpegkit.MediaInformationSession mediaInformationSession, final ExecutorService executorService, final int waitTimeout) {
        com.antonkarpenko.ffmpegkit.AsyncGetMediaInformationTask asyncGetMediaInformationTask = new com.antonkarpenko.ffmpegkit.AsyncGetMediaInformationTask(mediaInformationSession, waitTimeout);
     ...
     */
    public static void enableLogCallback(final com.antonkarpenko.ffmpegkit.LogCallback logCallback) {
     ...
     *                           defined callback
     */
    public static void enableStatisticsCallback(final com.antonkarpenko.ffmpegkit.StatisticsCallback statisticsCallback) {
     ...
     */
    public static void enableFFmpegSessionCompleteCallback(final com.antonkarpenko.ffmpegkit.FFmpegSessionCompleteCallback ffmpegSessionCompleteCallback) {
     ...
     */
    public static com.antonkarpenko.ffmpegkit.FFmpegSessionCompleteCallback getFFmpegSessionCompleteCallback() {
     ...
     *                                       previously defined callback
     */
    public static void enableFFprobeSessionCompleteCallback(final com.antonkarpenko.ffmpegkit.FFprobeSessionCompleteCallback ffprobeSessionCompleteCallback) {
     ...
     */
    public static com.antonkarpenko.ffmpegkit.FFprobeSessionCompleteCallback getFFprobeSessionCompleteCallback() {
     ...
     */
    public static void enableMediaInformationSessionCompleteCallback(final com.antonkarpenko.ffmpegkit.MediaInformationSessionCompleteCallback mediaInformationSessionCompleteCallback) {
     ...
     */
    public static com.antonkarpenko.ffmpegkit.MediaInformationSessionCompleteCallback getMediaInformationSessionCompleteCallback() {
     ...
     */
    public static com.antonkarpenko.ffmpegkit.Level getLogLevel() {
     ...
     */
    public static void setLogLevel(final com.antonkarpenko.ffmpegkit.Level level) {
        if (level != null) {
...
//            android.util.Log.e(TAG, String.format("Failed to close SAF fd: %d.%s", fileDescriptor, Exceptions.getStackTraceString(t)));
    ...
    private static void deleteExpiredSessions() {
        while (sessionHistoryList.size() > sessionHistorySize) {
            try {
                com.antonkarpenko.ffmpegkit.Session expiredSession = sessionHistoryList.remove(0);
     ...
     */
    static void addSession(final com.antonkarpenko.ffmpegkit.Session session) {
     ...
     */
    public static com.antonkarpenko.ffmpegkit.Session getSession(final long sessionId) {
        synchronized (sessionHistoryLock) {
     ...
     */
    public static com.antonkarpenko.ffmpegkit.Session getLastSession() {
        synchronized (sessionHistoryLock) {
     ...
     */
    public static com.antonkarpenko.ffmpegkit.Session getLastCompletedSession() {
        synchronized (sessionHistoryLock) {
            for (int i = sessionHistoryList.size() - 1; i >= 0; i--) {
                final com.antonkarpenko.ffmpegkit.Session session = sessionHistoryList.get(i);
                if (session.getState() == com.antonkarpenko.ffmpegkit.SessionState.COMPLETED) {
                    return session;
                }
            }
        }
     ...
     */
    public static List<com.antonkarpenko.ffmpegkit.Session> getSessions() {
        synchronized (sessionHistoryLock) {
     ...
     */
    public static List<com.antonkarpenko.ffmpegkit.FFmpegSession> getFFmpegSessions() {
        final LinkedList<com.antonkarpenko.ffmpegkit.FFmpegSession> list = new LinkedList<>();

        synchronized (sessionHistoryLock) {
            for (com.antonkarpenko.ffmpegkit.Session session : sessionHistoryList) {
                if (session.isFFmpeg()) {
                    list.add((com.antonkarpenko.ffmpegkit.FFmpegSession) session);
                }
            }
        }
     ...
     */
    public static List<com.antonkarpenko.ffmpegkit.FFprobeSession> getFFprobeSessions() {
        final LinkedList<com.antonkarpenko.ffmpegkit.FFprobeSession> list = new LinkedList<>();

        synchronized (sessionHistoryLock) {
            for (com.antonkarpenko.ffmpegkit.Session session : sessionHistoryList) {
                if (session.isFFprobe()) {
                    list.add((com.antonkarpenko.ffmpegkit.FFprobeSession) session);
                }
            }
        }
     ...
     */
    public static List<com.antonkarpenko.ffmpegkit.MediaInformationSession> getMediaInformationSessions() {
        final LinkedList<com.antonkarpenko.ffmpegkit.MediaInformationSession> list = new LinkedList<>();

        synchronized (sessionHistoryLock) {
            for (com.antonkarpenko.ffmpegkit.Session session : sessionHistoryList) {
                if (session.isMediaInformation()) {
                    list.add((com.antonkarpenko.ffmpegkit.MediaInformationSession) session);
                }
            }
        }
     ...
     */
    public static List<com.antonkarpenko.ffmpegkit.Session> getSessionsByState(final com.antonkarpenko.ffmpegkit.SessionState state) {
        final LinkedList<com.antonkarpenko.ffmpegkit.Session> list = new LinkedList<>();
     ...
     */
    public static com.antonkarpenko.ffmpegkit.LogRedirectionStrategy getLogRedirectionStrategy() {
     ...
     */
    public static void setLogRedirectionStrategy(final com.antonkarpenko.ffmpegkit.LogRedirectionStrategy logRedirectionStrategy) {
     ...
     * @return string value
     */
    public static String sessionStateToString(final com.antonkarpenko.ffmpegkit.SessionState state) {
