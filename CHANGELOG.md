...
* Added **Windows** support (x86_64) for the `https-gpl` FFmpeg 8.0 variant (gnutls TLS stack + the four GPL video libraries: gmp, gnutls, x264, x265, xvid, vid.stab).
* Native libraries are downloaded automatically at build time from the `8.0.0-https-gpl` release (`ffmpeg-kit-windows-x86_64-https-gpl-8.0.0.zip`); for local development point `FFMPEGKIT_LOCAL_DIR` (CMake cache var or environment) at a locally built bundle.
